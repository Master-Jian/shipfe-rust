...
fn run() -> Result<(), LicenseError> {
    ...
    match cli.command {
        ...
        Commands::Deploy {
            ...
            let global_config: crate::config::GlobalConfig = serde_json::from_str(&std::fs::read_to_string(&config_path)
                .map_err(|e| LicenseError::Invalid(format!("Failed to read config {}: {}", config_path, e)))?)
                .map_err(|e| LicenseError::Invalid(format!("config parse error: {e}")))?;

            // 支持 profile-sub 格式，如 dev-admin
            let (profile, sub) = if profile.contains('-') {
            ...
            let env_config = global_config.environments.get(&profile)
                .ok_or_else(|| LicenseError::Invalid(format!("Environment '{}' not found in config", profile)))?;

            let final_config = if let Some(sub_name) = &sub {
                if let Some(sub_envs) = &env_config.sub_environments {
                    let sub_config = sub_envs.get(sub_name)
                        .ok_or_else(|| LicenseError::Invalid(format!("Sub-environment '{}' not found in '{}'", sub_name, profile)))?;
                    ...
                    crate::config::DeployParams {
                        build_command: sub_config.build_command.clone().or_else(|| env_config.build_command.clone()),
                        local_dist_path: sub_config.local_dist_path.clone().unwrap_or_else(|| env_config.local_dist_path.clone()),
                        servers,
                        remote_tmp: env_config.remote_tmp.clone(),
                    }
                } else {
                    return Err(LicenseError::Invalid(format!("No sub-environments defined for '{}'", profile)));
                }
            } else {
                crate::config::DeployParams {
            ...
            println!("deploy profile={}{}, config={}", profile, sub.as_ref().map(|s| format!("-{}", s)).unwrap_or_default(), config_path);
            Ok(())
        }
